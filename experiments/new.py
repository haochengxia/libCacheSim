...
def sim_s4fifo(trace_name: str, cache_size_ratio: float = 0.01, fifo_size_ratio: float = 0.1, ghost_size_ratio: float = 0.9, move_to_main_threshold: int = 2, small_skip_ratio: float = 0):
...
def run_s4fifo(cache_size_ratio, trace_name, small_skip_ratio, move_to_main_threshold, fifo_size_ratio, additional_ghost_size_ratio):
    ...
    return (trace_name, cache_size_ratio, small_skip_ratio, move_to_main_threshold, fifo_size_ratio, additional_ghost_size_ratio, miss_ratio)
...
cdn2,2021_cdn2/1M/cf_allcolo.ns270.oracleGeneral.zst
    ...
    print(f"trace_names: {trace_names}")
    print(f"cache_size_ratio_grid: {cache_size_ratio_grid}")
    print(f"small_skip_ratio_grid: {small_skip_ratio_grid}")
    print(f"move_to_main_threshold_grid: {move_to_main_threshold_grid}")
    print(f"fifo_size_ratio_grid: {fifo_size_ratio_grid}")
    print(f"additional_ghost_size_ratio_grid: {additional_ghost_size_ratio_grid}")

    # 网格搜索的所有参数组合
    param_combinations = list(product(
        cache_size_ratio_grid,
        trace_names,
        small_skip_ratio_grid,
        move_to_main_threshold_grid,
        fifo_size_ratio_grid,
        additional_ghost_size_ratio_grid,
    ))

    # 多进程并行处理
    with mp.Pool(processes=mp.cpu_count()) as pool:
        results = pool.starmap(run_s4fifo, param_combinations)
    ...
    with open('baseline_results.csv', 'w') as f:
        ...
        for eviction_policy in [LRU, ARC, TwoQ, ThreeLCache, TinyLFU]:
