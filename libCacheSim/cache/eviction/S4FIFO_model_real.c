...
//
// Binary format written by scripts/s4fifo_export_model.py (kept alongside
...
typedef struct {
  ...
  double *cost_matrix;        // [n_classes * n_classes], row-major
} s4fifo_real_model_t;
...
static s4fifo_real_model_t *load_model_file(const char *path) {
  ...
  model->n_estimators = malloc(model->n_models * sizeof(uint32_t));
  if (!read_exact(fp, model->n_estimators,
                  model->n_models * sizeof(uint32_t))) {
    WARN("S4FIFO: %s truncated (n_estimators table)\n", path);
    goto out;
  }

  model->trees = malloc(model->n_trees_total * sizeof(s4fifo_real_tree_t));
  if (!read_exact(fp, model->trees,
                  model->n_trees_total * sizeof(s4fifo_real_tree_t))) {
    WARN("S4FIFO: %s truncated (tree table)\n", path);
    goto out;
  }

  const s4fifo_real_tree_t *last = &model->trees[model->n_trees_total - 1];
  uint64_t n_nodes = (uint64_t)last->node_offset + last->n_internal;
  uint64_t n_leaves = (uint64_t)last->leaf_offset + last->n_internal + 1;

  model->nodes = malloc(n_nodes * sizeof(s4fifo_real_node_t));
  if (!read_exact(fp, model->nodes, n_nodes * sizeof(s4fifo_real_node_t))) {
    WARN("S4FIFO: %s truncated (node array)\n", path);
    goto out;
  }

  model->leaves = malloc(n_leaves * sizeof(double));
  if (!read_exact(fp, model->leaves, n_leaves * sizeof(double))) {
...
out:
  fclose(fp);
  if (!ok) {
    free(model->n_estimators);
    free(model->trees);
    free(model->nodes);
    free(model->leaves);
    free(model->cost_matrix);
    free(model);
    return NULL;
  }
  return model;
