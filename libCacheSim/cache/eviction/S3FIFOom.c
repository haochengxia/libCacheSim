//  different from S3FIFOo which always inserts obj to small queue, when small queue is full but main queue is not full, directly insert obj to the main queue
...
//  libCacheSim
//
//  A variant of S3FIFO, ensuring at most one obj moved from 
//  fifo to main upon a request
...
// ****                                                               ****
// ***********************************************************************
cache_t *S3FIFOom_init(const common_cache_params_t ccache_params,
                     const char *cache_specific_params);
static void S3FIFOom_free(cache_t *cache);
static bool S3FIFOom_get(cache_t *cache, const request_t *req);

static cache_obj_t *S3FIFOom_find(cache_t *cache, const request_t *req,
                                const bool update_cache);
static cache_obj_t *S3FIFOom_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFOom_to_evict(cache_t *cache, const request_t *req);
static void S3FIFOom_evict(cache_t *cache, const request_t *req);
static bool S3FIFOom_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3FIFOom_get_occupied_byte(const cache_t *cache);
static inline int64_t S3FIFOom_get_n_obj(const cache_t *cache);
static inline bool S3FIFOom_can_insert(cache_t *cache, const request_t *req);
static void S3FIFOom_parse_params(cache_t *cache,
                                const char *cache_specific_params);

static void S3FIFOom_evict_fifo(cache_t *cache, const request_t *req);
static void S3FIFOom_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFOom_init(const common_cache_params_t ccache_params,
                     const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S3FIFOom", ccache_params, cache_specific_params);
  cache->cache_init = S3FIFOom_init;
  cache->cache_free = S3FIFOom_free;
  cache->get = S3FIFOom_get;
  cache->find = S3FIFOom_find;
  cache->insert = S3FIFOom_insert;
  cache->evict = S3FIFOom_evict;
  cache->remove = S3FIFOom_remove;
  cache->to_evict = S3FIFOom_to_evict;
  cache->get_n_obj = S3FIFOom_get_n_obj;
  cache->get_occupied_byte = S3FIFOom_get_occupied_byte;
  cache->can_insert = S3FIFOom_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3FIFOom_params_t));
  memset(cache->eviction_params, 0, sizeof(S3FIFOom_params_t));
  S3FIFOom_params_t *params = (S3FIFOom_params_t *)cache->eviction_params;
  params->req_local = new_request();
  params->hit_on_ghost = false;
  params->start_evict = false;

  S3FIFOom_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3FIFOom_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3FIFOom-%.4lf-%d",
           params->fifo_size_ratio, params->move_to_main_threshold);
  
  // // hack cache size
  // cache->cache_size = params->fifo->cache_size
  return cache;
}
...
static cache_obj_t *S3FIFOom_insert(cache_t *cache, const request_t *req) {
  ...
  if (params->hit_on_ghost) {
    ...
    // Case 1: admit-to-main, evictM
    while (params->main_cache->get_occupied_byte(params->main_cache) + req->obj_size + cache->obj_md_size > 
        params->main_cache->cache_size) {
        S3FIFOom_evict_main(cache, req);
    }
    ...
    if (!params->start_evict && params->fifo->get_occupied_byte(params->fifo) >= params->fifo->cache_size) {
      params->n_obj_admit_to_main += 1;
      params->n_byte_admit_to_main += req->obj_size;
      obj = params->main_cache->insert(params->main_cache, req);
    } else {
...
static void S3FIFOom_evict_fifo(cache_t *cache, const request_t *req) {
  ...
  while (!has_evicted && fifo->get_occupied_byte(fifo) > 0) {
    ...
    if (obj_to_evict->S3FIFO.freq >= params->move_to_main_threshold) {
      ...
      // Case 2: move-to-main, evictM
      while (main->get_occupied_byte(main) + params->req_local->obj_size + cache->obj_md_size > 
          main->cache_size) {
        S3FIFOom_evict_main(cache, params->req_local);
      }
      has_evicted = true;
...
static void S3FIFOom_evict(cache_t *cache, const request_t *req) {
  S3FIFOom_params_t *params = (S3FIFOom_params_t *)cache->eviction_params;
  params->start_evict = true;
  // if hit-on-ghost, do not need to evict from fifo
  // printf("hitn on ghost %d\n", params->hit_on_ghost);
  if (!params->hit_on_ghost) return S3FIFOom_evict_fifo(cache, req);
}
...
static inline int64_t S3FIFOom_get_occupied_byte(const cache_t *cache) {
  S3FIFOom_params_t *params = (S3FIFOom_params_t *)cache->eviction_params;
  if (params->hit_on_ghost) return 0;
...
// ****                                                               ****
// ***********************************************************************
static const char *S3FIFOom_current_params(S3FIFOom_params_t *params) {
...
static void S3FIFOom_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0) {
      ...
      printf("parameters: %s\n", S3FIFOom_current_params(params));
      exit(0);
    } else {
