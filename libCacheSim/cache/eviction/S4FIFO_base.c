...
static bool S4FIFO_base_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S4FIFO_base_get_occupied_byte(const cache_t *cache);
static inline int64_t S4FIFO_base_get_n_obj(const cache_t *cache);
static inline bool S4FIFO_base_can_insert(cache_t *cache, const request_t *req);
...
cache_t *S4FIFO_base_init(const common_cache_params_t ccache_params,
  ...
  cache->remove = S4FIFO_base_remove;
  cache->to_evict = S4FIFO_base_to_evict;
  cache->get_n_obj = S4FIFO_base_get_n_obj;
  cache->get_occupied_byte = S4FIFO_base_get_occupied_byte;
  cache->can_insert = S4FIFO_base_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S4FIFO_base_params_t));
  memset(cache->eviction_params, 0, sizeof(S4FIFO_base_params_t));
  S4FIFO_base_params_t *params = (S4FIFO_base_params_t *)cache->eviction_params;
...
// ****                                                               ****
// ***********************************************************************
static const char *S4FIFO_base_current_params(S4FIFO_base_params_t *params) {
...
static void S4FIFO_base_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0 ||
      ...
      printf("parameters: %s\n", S4FIFO_base_current_params(params));
      exit(0);
    } else {
