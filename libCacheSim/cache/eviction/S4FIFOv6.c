//
// S4FIFOv6 adds an explicit state transition step when parameters change.
...
// ****                                                               ****
// ***********************************************************************
static void S4FIFOv6_free(cache_t *cache);
static bool S4FIFOv6_get(cache_t *cache, const request_t *req);

static cache_obj_t *S4FIFOv6_find(cache_t *cache, const request_t *req,
                                const bool update_cache);
static cache_obj_t *S4FIFOv6_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S4FIFOv6_to_evict(cache_t *cache, const request_t *req);
static void S4FIFOv6_evict(cache_t *cache, const request_t *req);
static bool S4FIFOv6_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S4FIFOv6_get_occupied_byte(const cache_t *cache);
static inline int64_t S4FIFOv6_get_n_obj(const cache_t *cache);
static inline bool S4FIFOv6_can_insert(cache_t *cache, const request_t *req);
static void S4FIFOv6_parse_params(cache_t *cache,
                                  const char *cache_specific_params);

static void S4FIFOv6_evict_small(cache_t *cache, const request_t *req);
static void S4FIFOv6_evict_main(cache_t *cache, const request_t *req);
static void S4FIFOv6_apply_adjustment(cache_t *cache);
static void S4FIFOv6_clear_fifo(cache_t *fifo, request_t *req_local);
static int64_t S4FIFOv6_snapshot_fifo(cache_t *fifo, request_t **snapshot_out);
static void S4FIFOv6_rebuild_fifo(cache_t *fifo, request_t *snapshot,
...
cache_t *S4FIFOv6_init(const common_cache_params_t ccache_params,
                       const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S4FIFOv6", ccache_params, cache_specific_params);
  cache->cache_init = S4FIFOv6_init;
  cache->cache_free = S4FIFOv6_free;
  cache->get = S4FIFOv6_get;
  cache->find = S4FIFOv6_find;
  cache->insert = S4FIFOv6_insert;
  cache->evict = S4FIFOv6_evict;
  cache->remove = S4FIFOv6_remove;
  cache->to_evict = S4FIFOv6_to_evict;
  cache->get_n_obj = S4FIFOv6_get_n_obj;
  cache->get_occupied_byte = S4FIFOv6_get_occupied_byte;
  cache->can_insert = S4FIFOv6_can_insert;
  ...
  S4FIFOv6_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S4FIFOv6_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S4FIFOv6-%.4lf-%d",
...
static bool S4FIFOv6_get(cache_t *cache, const request_t *req) {
  ...
  if (params->request_count >= params->after_n_reqs && !params->has_adjusted) {
    S4FIFOv6_apply_adjustment(cache);
  }
...
static bool S4FIFOv6_insert_if_room(cache_t *fifo, const request_t *req) {
  if (fifo == NULL) {
    return false;
  }
...
static void S4FIFOv6_apply_adjustment(cache_t *cache) {
  ...
  int64_t n_small = S4FIFOv6_snapshot_fifo(params->small_fifo, &small_snapshot);
  int64_t n_main = S4FIFOv6_snapshot_fifo(params->main_fifo, &main_snapshot);
  int64_t protect_main_start = n_main / 2;
  ...
  S4FIFOv6_clear_fifo(params->small_fifo, params->req_local);
  S4FIFOv6_clear_fifo(params->main_fifo, params->req_local);
  S4FIFOv6_clear_fifo(params->ghost_fifo, params->req_local);
  params->s_counter = 0;

  for (int64_t i = 0; i < n_main; i++) {
    cache_obj_t *obj = NULL;
    bool protect_in_main = i >= protect_main_start;
    if (protect_in_main &&
        S4FIFOv6_insert_if_room(params->main_fifo, &main_snapshot[i])) {
      ...
      if (obj != NULL) {
        obj->S4FIFO.freq = 1;
               ...
               S4FIFOv6_insert_if_room(params->small_fifo, &main_snapshot[i])) {
  ...
  S4FIFOv6_rebuild_fifo(params->small_fifo, small_snapshot, n_small, true, params);

  free(small_snapshot);
  free(main_snapshot);
  params->has_adjusted = true;
  params->transition_type = 6;
...
static void S4FIFOv6_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    S4FIFOv6_evict_main(cache, req);
  } else {
    S4FIFOv6_evict_small(cache, req);
  }
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S4FIFOv6_current_params(S4FIFO_params_t *params) {
...
static void S4FIFOv6_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0 ||
      ...
      printf("parameters: %s\n", S4FIFOv6_current_params(params));
      exit(0);
    } else {
