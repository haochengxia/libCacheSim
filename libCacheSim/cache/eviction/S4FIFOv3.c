//
// S4FIFOv3 adds an explicit state transition step when parameters change.
...
// ****                                                               ****
// ***********************************************************************
static void S4FIFOv3_free(cache_t *cache);
static bool S4FIFOv3_get(cache_t *cache, const request_t *req);

static cache_obj_t *S4FIFOv3_find(cache_t *cache, const request_t *req,
                                const bool update_cache);
static cache_obj_t *S4FIFOv3_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S4FIFOv3_to_evict(cache_t *cache, const request_t *req);
static void S4FIFOv3_evict(cache_t *cache, const request_t *req);
static bool S4FIFOv3_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S4FIFOv3_get_occupied_byte(const cache_t *cache);
static inline int64_t S4FIFOv3_get_n_obj(const cache_t *cache);
static inline bool S4FIFOv3_can_insert(cache_t *cache, const request_t *req);
static void S4FIFOv3_parse_params(cache_t *cache,
                                  const char *cache_specific_params);

static void S4FIFOv3_evict_small(cache_t *cache, const request_t *req);
static void S4FIFOv3_evict_main(cache_t *cache, const request_t *req);
static void S4FIFOv3_apply_adjustment(cache_t *cache);
static void S4FIFOv3_clear_fifo(cache_t *fifo, request_t *req_local);
static int64_t S4FIFOv3_snapshot_fifo(cache_t *fifo, request_t **snapshot_out);
static void S4FIFOv3_rebuild_fifo(cache_t *fifo, request_t *snapshot,
...
cache_t *S4FIFOv3_init(const common_cache_params_t ccache_params,
                       const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S4FIFOv3", ccache_params, cache_specific_params);
  cache->cache_init = S4FIFOv3_init;
  cache->cache_free = S4FIFOv3_free;
  cache->get = S4FIFOv3_get;
  cache->find = S4FIFOv3_find;
  cache->insert = S4FIFOv3_insert;
  cache->evict = S4FIFOv3_evict;
  cache->remove = S4FIFOv3_remove;
  cache->to_evict = S4FIFOv3_to_evict;
  cache->get_n_obj = S4FIFOv3_get_n_obj;
  cache->get_occupied_byte = S4FIFOv3_get_occupied_byte;
  cache->can_insert = S4FIFOv3_can_insert;
  ...
  S4FIFOv3_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S4FIFOv3_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S4FIFOv3-%.4lf-%d",
...
static bool S4FIFOv3_get(cache_t *cache, const request_t *req) {
  ...
  if (params->request_count >= params->after_n_reqs && !params->has_adjusted) {
    S4FIFOv3_apply_adjustment(cache);
  }
...
static void S4FIFOv3_apply_adjustment(cache_t *cache) {
  ...
  int64_t n_small = S4FIFOv3_snapshot_fifo(params->small_fifo, &small_snapshot);
  int64_t n_main = S4FIFOv3_snapshot_fifo(params->main_fifo, &main_snapshot);
  ...
  S4FIFOv3_clear_fifo(params->small_fifo, params->req_local);
  S4FIFOv3_clear_fifo(params->main_fifo, params->req_local);
  S4FIFOv3_clear_fifo(params->ghost_fifo, params->req_local);
  params->s_counter = 0;

  S4FIFOv3_rebuild_fifo(params->small_fifo, small_snapshot, n_small, true,
                        params);
  S4FIFOv3_rebuild_fifo(params->main_fifo, main_snapshot, n_main, false,
                        params);

  free(small_snapshot);
  free(main_snapshot);
  params->has_adjusted = true;
  params->transition_type = 2;
...
static void S4FIFOv3_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    S4FIFOv3_evict_main(cache, req);
  } else {
    S4FIFOv3_evict_small(cache, req);
  }
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S4FIFOv3_current_params(S4FIFO_params_t *params) {
...
static void S4FIFOv3_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0 ||
      ...
      printf("parameters: %s\n", S4FIFOv3_current_params(params));
      exit(0);
    } else {
