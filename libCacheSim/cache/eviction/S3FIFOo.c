...
static void S3FIFOo_free(cache_t *cache);
static bool S3FIFOo_get(cache_t *cache, const request_t *req);

static cache_obj_t *S3FIFOo_find(cache_t *cache, const request_t *req,
                                const bool update_cache);
static cache_obj_t *S3FIFOo_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFOo_to_evict(cache_t *cache, const request_t *req);
static void S3FIFOo_evict(cache_t *cache, const request_t *req);
static bool S3FIFOo_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3FIFOo_get_occupied_byte(const cache_t *cache);
static inline int64_t S3FIFOo_get_n_obj(const cache_t *cache);
static inline bool S3FIFOo_can_insert(cache_t *cache, const request_t *req);
static void S3FIFOo_parse_params(cache_t *cache,
                                const char *cache_specific_params);

static void S3FIFOo_evict_fifo(cache_t *cache, const request_t *req);
static void S3FIFOo_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFOo_init(const common_cache_params_t ccache_params,
                     const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S3FIFOo", ccache_params, cache_specific_params);
  cache->cache_init = S3FIFOo_init;
  cache->cache_free = S3FIFOo_free;
  cache->get = S3FIFOo_get;
  cache->find = S3FIFOo_find;
  cache->insert = S3FIFOo_insert;
  cache->evict = S3FIFOo_evict;
  cache->remove = S3FIFOo_remove;
  cache->to_evict = S3FIFOo_to_evict;
  cache->get_n_obj = S3FIFOo_get_n_obj;
  cache->get_occupied_byte = S3FIFOo_get_occupied_byte;
  cache->can_insert = S3FIFOo_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3FIFOo_params_t));
  memset(cache->eviction_params, 0, sizeof(S3FIFOo_params_t));
  S3FIFOo_params_t *params = (S3FIFOo_params_t *)cache->eviction_params;
  params->req_local = new_request();
  params->hit_on_ghost = false;

  S3FIFOo_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3FIFOo_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3FIFOo-%.4lf-%d",
  ...
  while (!has_evicted && fifo->get_occupied_byte(fifo) > 0) {
    ...
    if (obj_to_evict->S3FIFO.freq >= params->move_to_main_threshold) {
      ...
      while (main->get_occupied_byte(main) + params->req_local->obj_size + cache->obj_md_size > 
          main->cache_size) {
        S3FIFOo_evict_main(cache, params->req_local);
      }
      has_evicted = true;
...
static void S3FIFOo_evict(cache_t *cache, const request_t *req) {
  ...
  if (!params->hit_on_ghost) return S3FIFOo_evict_fifo(cache, req);
}
...
static inline int64_t S3FIFOo_get_occupied_byte(const cache_t *cache) {
  ...
  return params->fifo->get_occupied_byte(params->fifo) + params->main_cache->cache_size;
  // params->main_cache->get_occupied_byte(params->main_cache);
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S3FIFOo_current_params(S3FIFOo_params_t *params) {
